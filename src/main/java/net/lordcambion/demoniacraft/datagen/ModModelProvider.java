...
public class ModModelProvider extends ModelProvider {
    ...
    protected Stream<Block> getKnownBlocks() {
        return ModBlocks.BLOCKS.getEntries().stream()
                ...
                .filter(item -> !(item instanceof ChairBlock))
                .filter(item -> !(item instanceof PedestalBlock));
    }
    ...
    protected Stream<Item> getKnownItems() {
        return ModItems.ITEMS.getEntries().stream()
                .map(RegistryObject::get)
                .filter(item -> !(item instanceof net.minecraft.world.item.SpawnEggItem)) // Escludi spawn eggs
                .filter(item -> !(item instanceof UndeadStaff)) // Escludi spawn eggs
                .filter(item -> !(item instanceof DemonScythe)) // Escludi spawn eggs
                .filter(item -> !(item instanceof ShockwaveStaff)); // Escludi spawn eggs
    }
